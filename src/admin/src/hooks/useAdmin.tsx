...
interface UseAdminReturn {
  ...
  queryRemittanceOrders: (filter?: {
    status?: ("AwaitingPayment" | "PaymentSubmitted" | "PaymentValidated" | "Cancelled" | "Settled")[];
  ...
  validateRemittancePayment: (orderId: string, approved: boolean, reason?: string) => Promise<void>;
  cancelRemittanceOrder: (orderId: string) => Promise<void>;
  getProviderDashboard: (providerId: string) => Promise<FrontendProviderDashboard>;
  getProviderAnalytics: (providerId: string, fromDate?: Date, toDate?: Date) => Promise<FrontendProviderAnalytics>;
  generateSettlementInstruction: (orderId: string) => Promise<FrontendSettlementInstruction>;
...
export const useAdmin = (): UseAdminReturn => {
  ...
  const [remittanceProviders, setRemittanceProviders] = useState<RemittanceServiceProviderData[]>([]);
  ...
  const refreshRemittanceOrders = useCallback(async (showSuccessToast = false) => {
  ...
  const refreshRemittanceProviders = useCallback(async (showSuccessToast = false) => {
  ...
  const refreshRemittanceStats = useCallback(async (showSuccessToast = false) => {
  ...
  const queryRemittanceOrders = useCallback(async (filter?: {
    status?: ("AwaitingPayment" | "PaymentSubmitted" | "PaymentValidated" | "Cancelled" | "Settled")[];
    serviceProviderId?: string;
    fromDate?: Date;
    toDate?: Date;
  ...
  const getRemittanceOrder = useCallback(async (orderId: string) => {
    try {
  ...
  const validateRemittancePayment = useCallback(async (orderId: string, approved: boolean, reason?: string) => {
    updateLoadingState("paymentValidation", true);
    try {
      await remittanceServiceCanister.validatePaymentByAdmin(orderId, approved, reason);
      await refreshRemittanceOrders();
      toast.success(`Payment ${approved ? 'approved' : 'rejected'} successfully`);
    } catch (error) {
      handleError(error, `Failed to ${approved ? 'approve' : 'reject'} payment`);
    } finally {
      updateLoadingState("paymentValidation", false);
    }
  }, [updateLoadingState, handleError, refreshRemittanceOrders]);

  const cancelRemittanceOrder = useCallback(async (orderId: string) => {
    try {
  ...
  const getProviderDashboard = useCallback(async (providerId: string) => {
    try {
  ...
  const getProviderAnalytics = useCallback(async (providerId: string, fromDate?: Date, toDate?: Date) => {
    try {
  ...
  const generateSettlementInstruction = useCallback(async (orderId: string) => {
    try {
      ...
      const mockSettlement: FrontendSettlementInstruction = {
        ...
        instructions: "Please deposit the commission amount to the specified GCash account. Include the reference number in the transaction. Send proof of payment via the app.",
