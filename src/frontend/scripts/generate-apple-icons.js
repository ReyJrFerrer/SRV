...
async function main() {
  ...
  if (!fs.existsSync(svgPath)) {
    console.error("logo.svg not found in public/. Please add your logo.svg to src/frontend/public/logo.svg and re-run.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Missing dependency 'sharp'. Run 'npm install --workspace src/frontend sharp --save-dev' or install it in src/frontend and try again.");
    process.exit(1);
  }
  ...
  for (const s of sizes) {
    ...
    try {
      await sharp(svgPath).resize(s, s, { fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } }).png().toFile(out);
  ...
  try {
    await sharp(svgPath).resize(180, 180, { fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } }).png().toFile(fallback);
