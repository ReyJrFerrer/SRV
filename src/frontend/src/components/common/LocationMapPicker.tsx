...
const LocationMapPicker: React.FC<LocationMapPickerProps> = ({
  ...
  const composeFormattedWithPlace = (
    ...
    let formatted = (formattedAddress || "").trim().replace(/^\+[^,]+,\s*/i, "");
    if (!formatted) return rawName || "";
    if (rawName && rawName.trim() && !formatted.toLowerCase().includes(rawName.trim().toLowerCase())) {
  ...
  const processPlaceDetails = useCallback((place: google.maps.places.PlaceResult | google.maps.GeocoderResult | any) => {
    ...
    const structured: StructuredLocation = {
      ...
      address: composeFormattedWithPlace((place as any).name, place.formatted_address),
      rawName: (place as any).name,
      route: getComponent("route"),
      barangay: getComponent("sublocality_level_1") || getComponent("neighborhood"),
  ...
  const reverseGeocodeAndUpdate = useCallback((pos: { lat: number; lng: number }) => {
    ...
    geocoderRef.current?.geocode({ location: pos }, (results, status) => {
      if (status === "OK" && results?.[0]) {
        ...
        const structured = { lat: pos.lat, lng: pos.lng, address: fallbackAddress } as StructuredLocation;
        onChange(structured);
        persistLocation(structured);
      }
    });
  ...
  const onSelectPrediction = useCallback((place: google.maps.places.PlaceResult) => {
    setShowDropdown(false);
    setPredictions([]);
