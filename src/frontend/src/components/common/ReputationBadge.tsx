...
function getBadgeClasses(score: number | undefined | null) {
  ...
  if (score >= 50) return { bg: "bg-blue-100", text: "text-blue-800", border: "border-blue-300" };
  if (score >= 20) return { bg: "bg-yellow-100", text: "text-yellow-800", border: "border-yellow-300" };
...
const ReputationBadge: React.FC<ReputationBadgeProps> = ({ score }) => {
  ...
  const display = typeof score === "number" && !isNaN(score) ? Math.round(score) : "–";

  return (
    <div
      title={typeof display === "number" ? `Reputation ${display}` : "No reputation"}
