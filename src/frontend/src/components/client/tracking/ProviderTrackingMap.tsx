...
const ProviderTrackingMap: React.FC<ProviderTrackingMapProps> = ({
    ...
    useEffect(() => {
        if (!providerLocation || !clientLocation || !(window as any).google?.maps) return;
    ...
    useEffect(() => {
        const map = mapRef.current;
        if (!map || !providerLocation || !clientLocation || !(window as any).google?.maps) return;
        ...
        return (
            <div className={`flex items-center justify-center bg-gray-100 ${className}`}>
                <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-600">Waiting for provider location...</p>
                </div>
            </div>
        );
    }
    ...
    const distanceText = directionsResult?.routes[0]?.legs[0]?.distance?.text || null;
