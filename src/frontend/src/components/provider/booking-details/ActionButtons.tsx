...
const ActionButtons: React.FC<Props> = ({
  ...
  const showDecline = !!(booking?.canAccept && booking?.canDecline && !isBookingActionInProgress(booking?.id || "", "decline"));
  const showAccept = !!(booking?.canAccept && booking?.canDecline && !acceptDisabledBecauseCommission && !isBookingActionInProgress(booking?.id || "", "accept"));
  ...
  if (showDecline) {
    buttons.push(
      <button key="decline" onClick={stopAndRun(onDecline)} className={`${baseButtonClass} w-full ${color.decline}`}>
  ...
  if (showAccept) {
    buttons.push(
      <button key="accept" onClick={stopAndRun(onAccept)} className={`${baseButtonClass} w-full ${color.accept}`}>
  ...
  if (showStart) {
    buttons.push(
      <button key="start" onClick={stopAndRun(onStart)} className={`${baseButtonClass} w-full ${color.start}`}>
  ...
  if (showComplete) {
    buttons.push(
      <button key="complete" onClick={stopAndRun(onComplete)} className={`${baseButtonClass} w-full ${color.complete}`}>
  ...
  if (showViewReview) {
    buttons.push(
      <Link key="viewReview" to={`/provider/review/${booking?.id}`} onClick={(e) => e.stopPropagation()} className={`${baseButtonClass} w-full ${color.review}`}>
