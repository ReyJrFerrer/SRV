...
const BottomNavigation: React.FC = () => {
  ...
  return (
    <div className="safe-area-inset-bottom fixed bottom-0 left-0 z-50 h-12 w-full border-t border-gray-200 bg-white sm:h-16">
      <div className="mx-auto grid h-full max-w-lg grid-cols-5 px-1 font-medium sm:px-0">
        {navItems.map((item) => {
          ...
          if (
            ...
            return (
              <Link
                key={item.label}
                to={item.to}
                className="group relative flex h-12 min-h-[44px] touch-manipulation flex-col items-center justify-center px-1 hover:bg-gray-50 sm:h-16 sm:px-3"
                onClick={(e) => {
                  if (isActive) {
                    e.preventDefault();
                    setTimeout(() => {
              ...
              >
                ...
                {item.count > 0 && (
                  <span className="absolute top-1 right-2 block h-2 w-2 rounded-full bg-red-500 sm:top-1 sm:right-4"></span>
                )}
              </Link>
            );
          }
          return (
            <Link
              key={item.label}
              to={item.to}
              className="group relative inline-flex h-12 min-h-[44px] touch-manipulation flex-col items-center justify-center px-1 hover:bg-gray-50 sm:h-16 sm:px-3"
              onClick={(e) => {
                if (isActive) {
                  e.preventDefault();
                  setTimeout(() => {
