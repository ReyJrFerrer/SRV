...
export const useCachedProviderBooking = (bookingId: string | undefined) => {
  const { getProviderBooking, isLoadingProviderBooking, isProviderBookingsReady } = useBookingCache();
  ...
  useEffect(() => {
    ...
    const fetchBooking = async () => {
      ...
      const minLoadingPromise = new Promise(resolve => setTimeout(resolve, 150));
  ...
  return {
    ...
    isLoading: isFetching || !minLoadingComplete || !isProviderBookingsReady || (bookingId ? isLoadingProviderBooking(bookingId) : false),
  };
};
...
export const useCachedClientBooking = (bookingId: string | undefined) => {
  const { getClientBooking, isLoadingClientBooking, isClientBookingsReady } = useBookingCache();
  ...
  return {
    booking,
    // Show loading if: fetching, minimum time not complete, or bookings not initialized yet
    isLoading: isFetching || !minLoadingComplete || !isClientBookingsReady || (bookingId ? isLoadingClientBooking(bookingId) : false),
  };
};
