...
export function useProviderLocationPublisher({
    ...
    const publishLocation = useCallback(
        async (location: google.maps.LatLngLiteral, heading?: number | null, speed?: number | null, accuracy?: number) => {
            ...
            try {
                ...
                console.error("[useProviderLocationPublisher] Failed to publish:", error);
            } finally {
    ...
    const forcePublish = useCallback(
        async (location: google.maps.LatLngLiteral, heading?: number | null, speed?: number | null, accuracy?: number) => {
            ...
            try {
                ...
                console.error("[useProviderLocationPublisher] Failed to force publish:", error);
            } finally {
                isPublishingRef.current = false;
            }
        },
