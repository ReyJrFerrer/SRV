...
const BookingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let attempts = 0; const iv = setInterval(() => {
      if ((window as any).google?.maps) { setMapsReady(true); clearInterval(iv); }
      else if (attempts++ > 50) { clearInterval(iv); }
    }, 200); return () => clearInterval(iv);
  }, []);

  // Reverse geocode detected coordinates
  useEffect(() => {
    if (mapMode !== "detected") return; if (detectedStatus === "loading" || detectedStatus === "ok") return;
    if (!geoLocation?.latitude || !geoLocation?.longitude) return; if (!mapsReady) return;
    try {
      if (!(window as any).google?.maps) return; setDetectedStatus("loading");
      ...
      geocoder.geocode({ location: loc }, (results: any, status: string) => {
        if (status === "OK" && results && results[0]) {
          const addr = results[0].formatted_address as string; setDetectedAddress(addr); setDetectedStatus("ok");
          setMapLocation((prev) => prev ?? { lat: loc.lat, lng: loc.lng, address: addr });
          if (!mapPreciseAddress) setMapPreciseAddress(addr); if (!mapDisplayAddress) setMapDisplayAddress(addr);
        } else { setDetectedStatus("failed"); }
      });
    } catch { setDetectedStatus("failed"); }
  ...
  useEffect(() => {
    if (manualProvince && manualCity) {
      const provinceObj = (phLocations as any).provinces.find((prov: any) => prov.name.trim().toLowerCase() === manualProvince.trim().toLowerCase());
      const muniObj = provinceObj?.municipalities.find((muni: any) => muni.name.trim().toLowerCase() === manualCity.trim().toLowerCase());
      ...
      } else { setManualBarangayOptions([]); }
      setSelectedBarangay("");
    } else { setManualBarangayOptions([]); setSelectedBarangay(""); }
  ...
  useEffect(() => {
      ...
      for (const prov of provinces) {
        for (const muni of prov.municipalities) {
          if (typeof muni === "object" && muni.name.trim().toLowerCase() === cityNorm && Array.isArray(muni.barangays)) {
            found = muni.barangays as string[]; matched = true; break;
          }
        }
        if (matched) break;
      }
    ...
    } else { setBarangayOptions([]); }
    setSelectedBarangay("");
  }, [displayMunicipality, displayProvince]);

  // Load service and packages
  useEffect(() => { if (serviceId) loadServiceData(serviceId); }, [serviceId, loadServiceData]);
  ...
  useEffect(() => {
    const run = async () => {
      if (service?.providerId) {
        ...
        } catch { setIsProviderOnboarded(false); }
      }
    }; run();
  }, [service?.providerId]);

  useEffect(() => { if (hookPackages.length > 0) setPackages(hookPackages.map((p: any) => ({ ...p, checked: false }))); }, [hookPackages]);

  // Load available slots when date/option changes
  useEffect(() => {
    if (!service) return;
    if (bookingOption === "scheduled" && selectedDate) { getAvailableSlots(service.id, selectedDate); }
    else if (bookingOption === "sameday") { const date = new Date(); getAvailableSlots(service.id, date); }
  ...
  useEffect(() => {
    const checkAll = async () => {
      ...
      setCheckingSlots(true); const map: Record<string, boolean> = {};
      const today = new Date();
      const dateToCheck = bookingOption === "sameday"
        ? new Date(today.getFullYear(), today.getMonth(), today.getDate(), 9, 0, 0, 0)
        : selectedDate;
      if (!dateToCheck) { setCheckingSlots(false); return; }
      const isTimeSlotPassed = (_start: string, end: string) => {
        if (bookingOption !== "sameday") return false; const now = new Date();
        const [eh, em] = end.split(":").map(Number); const slotEnd = new Date(); slotEnd.setHours(eh, em, 0, 0);
        return now >= slotEnd;
      };
      for (const slot of availableSlots) {
        ...
        if (passed) map[key] = false; else {
          try { const ok = await checkTimeSlotAvailability(service.id, dateToCheck, slot.timeSlot.startTime); map[key] = ok; }
          catch { map[key] = false; }
        }
      }
      setSlotAvailability(map); setCheckingSlots(false);
    };
    if (availableSlots.length > 0 && ((bookingOption === "scheduled" && selectedDate) || bookingOption === "sameday")) { checkAll(); }
  ...
  const hasUserBookedTimeSlot = useCallback((timeSlot: string, dateToCheck: Date): boolean => {
    ...
    const [startTimeStr] = timeSlot.split("-"); const targetDay = dateToCheck.toDateString();
    return userBookings.some((booking) => {
      if (["Cancelled","Declined","Completed"].includes(booking.status)) return false;
      if (booking.serviceId !== service.id) return false;
      const bookingDate = new Date(booking.scheduledDate); const bookingDay = bookingDate.toDateString();
  ...
  const handlePackageChange = (packageId: string) => { setPackages((prev) => prev.map((pkg) => pkg.id === packageId ? { ...pkg, checked: !pkg.checked } : pkg)); setFormError(null); };
  const handleBookingOptionChange = (option: "sameday" | "scheduled") => {
    setBookingOption(option); setSelectedTime(""); setSlotAvailability({}); setFormError(null);
    if (option === "scheduled" && service) { setSelectedDate(null); setTimeout(() => { const next = findNextAvailableBusinessDay(); if (next) setSelectedDate(next); }, 0); }
    else if (option === "sameday") { setSelectedDate(null); }
  };

  const dayIndexToName = (dayIndex: number): string => ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][dayIndex] || "";
  const findNextAvailableBusinessDay = (): Date | null => {
    if (!service?.weeklySchedule) return null; const today = new Date(); let currentDate = new Date(today.setDate(today.getDate() + 1));
    for (let i = 0; i < 14; i++) {
      const dayName = dayIndexToName(currentDate.getDay());
      const isAvailable = service.weeklySchedule.some((s) => s.day === dayName && s.availability.isAvailable);
      if (isAvailable) {
        return new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), 9, 0, 0, 0);
      }
  ...
  const handleDateChange = (date: Date | null) => { if (date) { const adjusted = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 9, 0, 0, 0); setSelectedDate(adjusted); } else { setSelectedDate(null); } setSelectedTime(""); setFormError(null); };
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => { const value = e.target.value; if (/^\d*\.?\d{0,2}$/.test(value)) setAmountPaid(value); setFormError(null); };
  const handleNotesChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => { setNotes(e.target.value); setFormError(null); };

  const [highlightInput, setHighlightInput] = useState<string>("");
  useEffect(() => {
    if (highlightInput === "barangay" && selectedBarangay && selectedBarangay !== "__other__") setHighlightInput("");
    if (highlightInput === "otherBarangay" && otherBarangay && otherBarangay.trim().length >= 3 && otherBarangay.trim().length <= 20) setHighlightInput("");
    if (highlightInput === "street" && street && street.trim().length >= 3 && street.trim().length <= 20) setHighlightInput("");
    if (highlightInput === "houseNumber" && houseNumber && houseNumber.length <= 15 && /\d/.test(houseNumber)) setHighlightInput("");
    if (highlightInput === "amountPaid" && amountPaid) setHighlightInput("");
    if (highlightInput === "selectedTime" && selectedTime) setHighlightInput("");
    if (highlightInput === "package" && packages.some((pkg) => pkg.checked)) setHighlightInput("");
    if (highlightInput === "bookingOption" && bookingOption) setHighlightInput("");
    if (highlightInput === "paymentSection" && !paymentError && amountPaid) setHighlightInput("");
  }, [highlightInput, selectedBarangay, otherBarangay, street, houseNumber, amountPaid, selectedTime, packages, bookingOption, paymentError]);

  useEffect(() => {
    if (window.innerWidth > 768) return; let ref: HTMLElement | null = null;
    ...
    if (highlightInput === "bookingOption" || highlightInput === "selectedTime") ref = bookingSectionRef.current as any;
    if (highlightInput === "paymentSection") ref = paymentSectionRef.current as any;
    if (ref) { setTimeout(() => { ref?.scrollIntoView({ behavior: "smooth", block: "center" }); }, 150); }
  }, [highlightInput]);

  const handleConfirmBooking = async () => {
    setFormError(null); setIsSubmitting(true); let highlightField = "";
    try {
      if (!packages.some((p) => p.checked)) { setFormError("Please select at least one package before proceeding."); highlightField = "package"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      if (!bookingOption) { setFormError("Please select a booking type (Same Day or Scheduled)."); highlightField = "bookingOption"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      if (!selectedTime) { const timeLabel = bookingOption === "sameday" ? "time for today" : "time slot"; setFormError(`Please select a ${timeLabel} before proceeding.`); highlightField = "selectedTime"; setIsSubmitting(false); setHighlightInput(highlightField); return; }

      const isUsingMapPin = !showFallbackForms && !!(mapLocation && mapLocation.lat && mapLocation.lng);
      if (!isUsingMapPin) {
        if (!selectedBarangay.trim()) { setFormError("Please select your Barangay before proceeding (or drop a map pin)."); highlightField = "barangay"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
        if (selectedBarangay === "__other__" && (!otherBarangay || otherBarangay.trim().length < 3 || otherBarangay.trim().length > 20)) { setFormError("Please enter a valid barangay name (3-20 characters) for 'Others'."); highlightField = "otherBarangay"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
        if (street.trim().length < 3 || street.trim().length > 20) { setFormError("Street Name must be between 3 and 20 characters."); highlightField = "street"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
        if (!houseNumber.trim() || houseNumber.length > 15 || !/\d/.test(houseNumber)) { setFormError("House/Unit No. must be at most 15 characters and contain at least one number."); highlightField = "houseNumber"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      }

      if (paymentMethod === "CashOnHand" && packages.some((pkg) => pkg.checked)) {
        const paidAmount = parseFloat(amountPaid);
        if (!amountPaid.trim()) { setFormError("Please enter the cash amount before proceeding."); highlightField = "paymentSection"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
        if (isNaN(paidAmount) || paidAmount < totalPrice) { setFormError(`Cash amount must be at least ₱${totalPrice.toFixed(2)}.`); highlightField = "paymentSection"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      }

      if (!packages.some((pkg) => pkg.checked)) { setFormError("Please select at least one package before proceeding."); highlightField = "package"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      if (!bookingOption) { setFormError("Please select a booking type (Same Day or Scheduled)."); highlightField = "bookingOption"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      if (!selectedTime) { const timeLabel = bookingOption === "sameday" ? "time for today" : "time slot"; setFormError(`Please select a ${timeLabel} before proceeding.`); highlightField = "selectedTime"; setIsSubmitting(false); setHighlightInput(highlightField); return; }

      const [, endTimeStr] = selectedTime.split("-"); if (!endTimeStr) { setFormError("Invalid time slot format. Expected format: HH:MM-HH:MM"); setIsSubmitting(false); return; }
      const [endHour, endMinute] = endTimeStr.split(":").map(Number); if (isNaN(endHour) || isNaN(endMinute)) { setFormError("Invalid time slot format."); setIsSubmitting(false); return; }
      let finalScheduledDate: Date;
      if (bookingOption === "sameday") {
        const today = new Date(); finalScheduledDate = new Date(today.getFullYear(), today.getMonth(), today.getDate(), endHour, endMinute, 0, 0);
      } else if (bookingOption === "scheduled" && selectedDate) {
        finalScheduledDate = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate(), endHour, endMinute, 0, 0);
      } else { setFormError("Please select a date for scheduled booking."); setIsSubmitting(false); return; }

      const barangayValue = selectedBarangay === "__other__" ? otherBarangay : selectedBarangay;
      const finalMunicipality = locationInputMode === "manual" ? manualCity : displayMunicipality;
      const finalProvince = locationInputMode === "manual" ? manualProvince : displayProvince;

      let finalAddress: string;
      if (!showFallbackForms && mapLocation?.lat && mapLocation?.lng) {
        let base = mapDisplayAddress || mapLocation.address?.trim() || "";
        const normalizedBase = base.toLowerCase();
        if (barangayValue && barangayValue.trim() && !normalizedBase.includes(barangayValue.trim().toLowerCase())) {
          ...
          } else { base = `${base}, ${barangayValue}`; }
        }
        const landmarkPart = landmark ? ` (${landmark})` : "";
        finalAddress = base ? `${base}${landmarkPart}` : `Pinned Location (${mapLocation.lat.toFixed(5)}, ${mapLocation.lng.toFixed(5)})${landmarkPart}`;
      } else {
        finalAddress = [houseNumber, street, barangayValue, finalMunicipality, finalProvince, landmark].filter(Boolean).join(", ");
      }
      ...
      const duplicateBooking = userBookings.find((booking) => {
        ...
        if (booking.serviceId !== service!.id) return false;
        const bookingDate = new Date(booking.scheduledDate); const bookingDay = bookingDate.toDateString();
        let targetDate: Date; if (bookingOption === "sameday") {
          const today = new Date(); targetDate = new Date(today.getFullYear(), today.getMonth(), today.getDate());
        } else { targetDate = new Date(selectedDate!.getFullYear(), selectedDate!.getMonth(), selectedDate!.getDate()); }
        const targetDay = targetDate.toDateString(); if (bookingDay !== targetDay) return false;
        const bookingRequestedDate = new Date(booking.requestedDate);
        const bookingStartTime = `${String(bookingRequestedDate.getHours()).padStart(2, "0")}:${String(bookingRequestedDate.getMinutes()).padStart(2, "0")}`;
        return bookingStartTime === startTimeStr.trim();
      });
      if (duplicateBooking) { setFormError(`You already have a ${duplicateBooking.status.toLowerCase()} booking for this service on ${new Date(duplicateBooking.scheduledDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} at ${selectedTime}. Please choose a different time slot or cancel your existing booking.`); highlightField = "selectedTime"; setIsSubmitting(false); setHighlightInput(highlightField); return; }
      ...
      if (mapLocation?.lat && mapLocation?.lng) {
        (bookingData as any).latitude = mapLocation.lat; (bookingData as any).longitude = mapLocation.lng;
        if (mapLocation.address) (bookingData as any).geocodedAddress = mapLocation.address;
        if (mapPreciseAddress) (bookingData as any).preciseAddress = mapPreciseAddress;
        if (mapDisplayAddress) (bookingData as any).displayAddress = mapDisplayAddress;
      }

      if (paymentMethod === "GCash") {
        if (!identity) { setFormError("You must be logged in to make digital payments."); return; }
        const isOnboarded = await checkProviderOnboarding(service!.providerId.toString());
        if (!isOnboarded) { setFormError("This provider hasn't set up direct payments yet. Please use cash payment or SRV Wallet."); return; }
        ...
        const paymentResult = await createDirectPayment({
          ...
          bookingData: { ...bookingData, location: typeof bookingData.location === "string" ? bookingData.location : finalAddress },
        });
        if (!paymentResult.success) { setFormError(paymentResult.error || "Failed to create payment invoice. Please try again."); return; }
        if (paymentResult.invoiceUrl) {
          window.open(paymentResult.invoiceUrl, "_blank");
          navigate("/client/booking/payment-pending", { state: { invoiceId: paymentResult.invoiceId, invoiceUrl: paymentResult.invoiceUrl, bookingData } });
          return;
        }
      }
      ...
      if (booking) {
        ...
        const confirmationDetails = {
          ...
          date: finalScheduledDate ? finalScheduledDate.toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" }) : "Same Day",
          time: bookingData.scheduledTime || "As soon as possible",
          packagePrice: totalPrice.toFixed(2),
          amountToPay: paymentMethod === "CashOnHand" ? amountPaid || totalPrice.toFixed(2) : totalPrice.toFixed(2),
          landmark: landmark || "None",
          municipality: finalMunicipality,
          province: finalProvince,
        };
        navigate("/client/booking/confirmation", { state: { details: confirmationDetails } });
      } else { setFormError("Failed to create booking. Please try again."); }
    } catch (error: any) {
      const errorMessage = error instanceof Error ? error.message : "An unknown error occurred.";
      setFormError(`An error occurred while creating the booking: ${errorMessage}`);
  ...
  if (error) return <div className="p-10 text-center text-red-500">{String(error)}</div>;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between px-4 py-3">
          <button onClick={() => navigate(-1)} className="text-2xl text-gray-600 hover:text-gray-800">←</button>
            ...
            <div className="mx-auto max-w-5xl px-2 py-8 md:px-0">
              <div className="md:flex md:gap-x-8">
                <div className="space-y-6 md:w-1/2">
                  <PackagesSection packages={packages} onToggle={handlePackageChange} highlight={highlightInput === "package"} innerRef={packageSectionRef} />
                ...
                <div className="mt-8 space-y-6 md:mt-0 md:w-1/2">
                  <ScheduleSection
                    ...
                    highlight={highlightInput === "bookingOption" || highlightInput === "selectedTime"}
                  />
                  ...
                  <NotesSection notes={notes} onChange={handleNotesChange} limit={NOTES_CHAR_LIMIT} />
                </div>
              </div>
            </div>
          </div>
          <StickyConfirmBar formError={formError} isSubmitting={isSubmitting} onConfirm={handleConfirmBooking} />
        </div>
      </main>
    </div>
  );
