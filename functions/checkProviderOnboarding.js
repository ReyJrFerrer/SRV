...
exports.checkProviderOnboarding = functions.https.onRequest(
  ...
  try {
    ...
    const providerId = req.method === "GET" 
    ...
    if (!xendit) {
      ...
      try {
        ...
        const isOnboarded = providerDoc.exists && 
          providerDoc.data() && 
        ...
        return res.status(200).json({
          result: {
            ...
            details: isOnboarded ? {
              xenditCustomerId: providerDoc.data() && providerDoc.data().xenditCustomerId,
              onboardedAt: providerDoc.data() && providerDoc.data().onboardedAt,
              payoutInfo: providerDoc.data() && providerDoc.data().payoutInfo,
            } : null,
          },
        });
    ...
    try {
      ...
      if (customerResponse && customerResponse.data && customerResponse.data.length > 0) {
    ...
    const isFirestoreOnboarded = firestoreData && 
    ...
    const details = isOnboarded ? {
      xenditCustomerId: (xenditCustomer && xenditCustomer.id) || (firestoreData && firestoreData.xenditCustomerId),
      xenditReferenceId: (xenditCustomer && xenditCustomer.referenceId) || providerId,
      onboardedAt: (firestoreData && firestoreData.onboardedAt) || (xenditCustomer && xenditCustomer.created),
