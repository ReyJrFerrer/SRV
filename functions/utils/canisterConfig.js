...
function loadCommissionIdlFactory() {
  try {
    ...
    const declarationsPath = path.resolve(
      __dirname,
      "../../src/declarations/commission/commission.did.js",
    );
    ...
    console.warn(
      "Commission declarations not found, using manual IDL definition",
    );
    return getManualCommissionIdl();
  } catch (error) {
    console.warn(
      "Failed to load commission declarations, using manual IDL:",
      error.message,
    );
    return getManualCommissionIdl();
  }
...
const CANISTER_IDS = {
  local: {
    commission:
      process.env.COMMISSION_CANISTER_ID_LOCAL || "by6od-j4aaa-aaaaa-qaadq-cai",
    wallet:
      process.env.WALLET_CANISTER_ID_LOCAL || "br5f7-7uaaa-aaaaa-qaaca-cai",
    booking:
      process.env.BOOKING_CANISTER_ID_LOCAL || "bkyz2-fmaaa-aaaaa-qaaaq-cai",
    backend:
      process.env.BACKEND_CANISTER_ID_LOCAL || "bkyz2-fmaaa-aaaaa-qaaaq-cai",
  },
  ic: {
...
async function createCanisterActor(canisterName, idlFactory) {
  ...
  if (!canisterId) {
    throw new Error(
      `Canister ID not found for ${canisterName} in ${config.environment} environment`,
    );
  }
...
const walletIdlFactory = ({ IDL }) => {
  ...
  return IDL.Service({
    ...
    credit: IDL.Func(
      [IDL.Principal, IDL.Nat, IDL.Opt(IDL.Text), IDL.Opt(IDL.Text)],
      [WalletResult],
      [],
    ),
    debit: IDL.Func(
...
async function getCommissionFromCanister(category, amount) {
  try {
    console.log(
      `Getting commission for category: ${category}, amount: ${amount}`,
    );

    const commissionActor = await createCanisterActor(
      "commission",
...
async function getCommissionBreakdown(category, amount) {
  try {
    console.log(
      `Getting commission breakdown for category: ${category}, amount: ${amount}`,
    );
    ...
    const result = await commissionActor.get_commission_breakdown(
      category,
      amount,
    );
...
async function validateCanisterConnection() {
  ...
  try {
    ...
    const commissionResult = await getCommissionFromCanister(
      "Cleaning Services",
      1000,
    );
