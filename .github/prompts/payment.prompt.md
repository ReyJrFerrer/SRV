...
**2.1. Create Off-Chain Service** - **Action:** CREATE the folder `src/xendit-service/` and add `index.js` and `package.json`. - **Content:** Set up a basic Node.js/Express server.

**2.2. Implement Xendit Endpoints** - **Action:** MODIFY `src/xendit-service/index.js`. - **Content:** Implement the following API endpoints: - `/onboard-provider`: To create a sub-account on xenPlatform for a provider. - `/create-direct-payment`: To generate a xenPlatform invoice for direct GCash payments with a `fee_rule` that uses the new commission logic. - `/create-topup-invoice`: To generate a standard Xendit invoice for SRV Wallet top-ups. - `/xendit-webhook`: A single webhook handler to process notifications.

**2.3. Connect Booking Canister to Service** - **Action:** MODIFY `src/backend/function/booking.mo`. - **Content:** Add a secure function `confirm_digital_payment(booking_id: Text)` that can only be called by the `xendit-service`'s Principal.

**2.4. Frontend Provider Onboarding** - **Action:** CREATE `src/frontend/src/pages/provider/payout-settings.tsx`. - **Content:** Build a secure form for providers to submit their GCash details, which calls the `/onboard-provider` endpoint.

**2.5. Frontend Payment Flow Integration** - **Action:** MODIFY `src/frontend/src/pages/client/book/[id].tsx`. - **Content:** Update the UI to present three clear payment options: "Pay with GCash", "Pay with SRV Wallet", and "Pay with Cash".

**2.6. Frontend Wallet Top-Up** - **Action:** MODIFY `src/frontend/src/pages/provider/wallet.tsx`. - **Content:** Connect the "Top Up" button to the `/create-topup-invoice` endpoint to enable automated wallet top-ups.
